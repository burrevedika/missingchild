...
def ParentLoginAction(request):
    if request.method == 'POST':
      ...
      password = request.POST.get('t2', False)
      index = 0
      con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'MissingChildDB',charset='utf8')
      with con:    
          cur = con.cursor()
...
def AdoptAction(request):
    if request.method == 'GET':
        ...
        db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'MissingChildDB',charset='utf8')
...
def UploadAction(request):
     ...
     if request.method == 'POST' and request.FILES['t5']:
        ...
        filename = fs.save('D:/MINI PROJECT/MissingChilds/MissingChildApp/static/photo/'+child_name+'.png', myfile)
        #if index == 0:
...
def ParentRegisterAction(request):
    if request.method == 'POST' and request.FILES['t9']:
        ...
        fs = FileSystemStorage()
        filename = fs.save('D:/MINI PROJECT/MissingChilds/MissingChildApp/static/documents/'+filenames, myfile)
